...
def guess_pairs_in_folder(folder: Path) -> List[Tuple[Path, Path, str]]:
    """Return list of tuples: (before_path, after_path, suggested_name_stem)
    Pairing heuristics:
      1) *_before.* with *_after.* (case-insensitive, -, _, space ok)
      2) fall back to alphabetical pairing (0-1, 2-3, ...)
    """
