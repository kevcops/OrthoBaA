...
def run_app():
    ...
    np = cfg.get("name_parts", {"use_id": True, "use_first": True, "use_last": True})
    win.name_id_cb.setChecked(bool(np.get("use_id", True)))
    win.name_first_cb.setChecked(bool(np.get("use_first", True)))
    win.name_last_cb.setChecked(bool(np.get("use_last", True)))

    def current_name_prefs():
        return {
            "use_id": win.name_id_cb.isChecked(),
            "use_first": win.name_first_cb.isChecked(),
            "use_last": win.name_last_cb.isChecked(),
        }

    def suggest_name():
        if not (win.before.path and win.after.path):
            return
        b_name = Path(win.before.path).name if win.before.path else ""
        a_name = Path(win.after.path).name if win.after.path else ""
        prefs = current_name_prefs()
        base = suggest_output_basename_from_two_with_prefs(
            b_name, a_name, prefs["use_id"], prefs["use_first"], prefs["use_last"]
        )
        ext = ".pdf" if win.format_combo.currentText() == "PDF" else ".jpg"
        win.filename.setText(base + ext)

    win.before.pathChanged.connect(lambda _: suggest_name())
    win.after.pathChanged.connect(lambda _: suggest_name())

    def on_parts_changed():
        cfg["name_parts"] = current_name_prefs()
        save_config(cfg)
        suggest_name()
    win.name_id_cb.stateChanged.connect(lambda _: on_parts_changed())
    win.name_first_cb.stateChanged.connect(lambda _: on_parts_changed())
    win.name_last_cb.stateChanged.connect(lambda _: on_parts_changed())
    ...
    def do_export(preview: bool = False):
        ...
        if not name:
            b = Path(win.before.path).name if win.before.path else ""
            a = Path(win.after.path).name if win.after.path else ""
            prefs = current_name_prefs()
            base = suggest_output_basename_from_two_with_prefs(
                b, a, prefs["use_id"], prefs["use_first"], prefs["use_last"]
            )
            name = base
    ...
    def do_batch():
        ...
        out_dir = Path(win.out_dir.text().strip() or cfg["last_out_dir"]); out_dir.mkdir(parents=True, exist_ok=True)
        fmt = win.format_combo.currentText(); scale = float(cfg.get("scale_factor", 0.85))
        ...
        for i, (b_path, a_path, stem) in enumerate(pairs, start=1):
            ...
            if fmt == "PDF": export_pdf(out_path.with_suffix(".pdf"), b_eff, a_eff, scale_factor=scale)
            else: export_jpeg(out_path.with_suffix(".jpg"), b_eff, a_eff, quality=92, scale_factor=scale)
            win.progress.setValue(int(i / len(pairs) * 100)); app.processEvents()

        win.status.showMessage("Batch complete."); cfg["last_out_dir"] = str(out_dir); cfg["output_format"] = fmt; cfg["name_parts"] = current_name_prefs(); save_config(cfg)
